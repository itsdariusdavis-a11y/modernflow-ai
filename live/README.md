...
- **GitHub Pages** — merge this to `main` and enable Pages (Settings → Pages →
  ...
  publishes `live/`. _Pages on a private repo needs a paid GitHub plan._
...
- **Locally** — just open `index.html`, or `npx serve live`.
...
1. **Server-side daily snapshot** — the GitHub Action
   (`.github/workflows/deploy.yml`) runs on a cron a few times a day,
   executes `scripts/build-snapshot.mjs` to pull the full slate from the
...
snapshot locally: `node scripts/build-snapshot.mjs` (needs internet).
